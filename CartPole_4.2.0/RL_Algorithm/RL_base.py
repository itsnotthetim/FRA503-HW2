...
class BaseAlgorithm():
    ...
    def discretize_state(self, obs: dict):
        ...
        # Get policy
        state = obs["policy"]  # shape: (1, 4)
        
        # If policy is a PyTorch tensor, convert to NumPy array
        if isinstance(state, torch.Tensor):
            state_array = state.cpu().numpy().flatten()  # Convert to NumPy array 1D
        else:
            state_array = np.array(state).flatten()

        # Scale and discretize using weights
        discrete_state = np.round([
            state_array[0] * self.discretize_state_weight[0], # pose_cart
            state_array[1] * self.discretize_state_weight[1], # pose_pole
            state_array[2] * self.discretize_state_weight[2], # vel_cart
            state_array[3] * self.discretize_state_weight[3]  # vel_pole
        ]).astype(int)  # Ensure integer output
    ...
    def mapping_action(self, action):
        ...
        """
        # ========= put your code here =========#
        # Extract the range [min, max]
        min_action = self.action_range[0]
        max_action = self.action_range[1]
    
        # Avoid division by zero if there's only one discrete action
        if self.num_of_action == 1:
            return torch.tensor(min_action, dtype=torch.float32)

        # Scale discrete action to continuous space
        continuous_action = min_action + (action / (self.num_of_action - 1)) * (max_action - min_action)
